...
NOTE: 이 방법은 저장된 소프트웨어 암호화를 사용하지 않습니다. 이 작업은 를 통해서만 수행할 수 있습니다 link:../api/reference_element_api_createcluster.html["클러스터 생성 방법"] 와 함께 `enableSoftwareEncryptionAtRest` 를 로 설정합니다 `true`.
