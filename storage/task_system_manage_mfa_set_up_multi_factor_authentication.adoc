...
+
IdP 메타데이터는 일반 텍스트 형식으로 타사 IdP에서 가져옵니다. 이 메타데이터는 JSON 형식으로 올바르게 포맷되었는지 확인하기 위해 유효성 검사가 필요합니다. 사용할 수 있는 JSON 포맷터 애플리케이션은 여러 가지가 있습니다. 예를 들면 다음과 같습니다:https://freeformatter.com/json-escape.html
